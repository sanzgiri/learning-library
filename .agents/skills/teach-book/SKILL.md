...
5. **Author lessons.** For each concept slated this pass, write `books/<id>/lessons/NNNN-slug.html` per [LESSON-FORMAT.md](./LESSON-FORMAT.md). *Done when:* the file links the shared assets via absolute paths (`/assets/lesson.css`, `/assets/chat.js` — never `../`-relative, see LESSON-FORMAT.md for why), contains every required section, and cross-links at least one related lesson.
...
8. **Register the book in the shared library.** Update this book's entry in `books/index.json` (title, author, tagline, `"status": "active"`), and confirm `books/<id>/index.html` exists as a dashboard for this book alone — copy an existing book's dashboard as the pattern rather than writing one from scratch, and keep its `assets/` references absolute, same as lessons. *Done when:* the root library page would list this book correctly with zero further edits.
